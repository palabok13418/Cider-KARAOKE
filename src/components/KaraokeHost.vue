...
import {
  ciderNowPlaying, ciderPlay, ciderSearch, demoLyrics, demoSongs, enhanceLyrics,
...
const query = ref("");
const results = ref<Song[]>(demoSongs);
...
function addToQueue(song: Song) {
  ...
  status.value = "Added " + song.title + " to the karaoke queue.";
}

async function playSelected(song: Song, remove = false) {
  current.value = song;
  lines.value = await enhanceLyrics(demoLyrics[song.id] || [
    {id:"fallback",original:"Ready to sing",language:"en",translation:"Ready to sing"}
  ]);
...
async function search() {
  ...
  if (!needle) { results.value = demoSongs; return; }
  searchBusy.value = true;
  const live = await ciderSearch(needle);
  results.value = live.length ? live : demoSongs.filter((song) => (song.title+" "+song.artist).toLowerCase().includes(needle.toLowerCase()));
  searchBusy.value = false;
}

function removeQueue(id: string) {
  queue.value = queue.value.filter((song) => song.queueId !== id);
}
...
function onMessage(message: Signal) {
  ...
  if (message.type === "peer-joined") {
    if (peers.has(message.peerId)) return;
    micCount.value += 1;
    void preparePeer(message.peerId);
    return;
  }
  if (message.type === "peer-left") {
    micCount.value = Math.max(0,micCount.value-1);
    peers.get(message.peerId)?.close();
    peers.delete(message.peerId);
    return;
  }
  if (message.type === "webrtc-offer") {
    void answerOffer(message.peerId,message.sdp);
    return;
  }
  if (message.type === "webrtc-ice") {
    const peer = peers.get(message.peerId);
    if (peer) void peer.addIceCandidate(message.candidate);
    return;
  }
...
async function preparePeer(peerId: string) {
  ...
  peer.onicecandidate = (event) => {
    if (event.candidate) transport.value?.send({type:"webrtc-ice",peerId,candidate:event.candidate.toJSON()});
  };
}
...
onMounted(async () => {
  ...
  await playSelected(now || demoSongs[0]);
});
    ...
    <div class="host-body">
      <section class="main-stage">
          ...
          <div class="stage-content">
            ...
            <div class="stage-footer">
              <div class="vocal-status">
                <div><span class="eyebrow">VOCAL REMOVER</span><strong>{{ vocal.mode.toUpperCase() }}</strong></div>
                <span class="status-dot" :class="{live:vocal.loaded}"></span>
                <small>{{ vocal.message }}</small>
              </div>
        ...
        <section class="browser-drawer">
          <div class="browser-header">
            <div><span class="eyebrow">APPLE MUSIC SING</span><h2>Karaoke Browser</h2></div>
            <span class="catalog-pill">Sing</span>
          </div>
          <div class="search-row"><input v-model="query" class="search" placeholder="Search Sing songs…" @keyup.enter="search" /><button class="primary-btn" @click="search">{{ searchBusy ? "Searching…" : "Search" }}</button></div>
          <div class="category-row"><button v-for="category in ['Sing','Popular','Rock','Pop','R&B','Classics','Disney']" :key="category" class="category">{{ category }}</button></div>
          <div class="song-list">
            <button v-for="song in results" :key="song.id" class="song-card" @click="addToQueue(song)">
              <img :src="song.artwork" :alt="song.title" />
              <span class="song-copy"><strong>{{ song.title }}</strong><small>{{ song.artist }}</small></span><span class="add-icon">＋</span>
            </button>
          </div>
        </section>
      </section>
      ...
      <aside class="queue-panel">
        ...
        <div class="queue-list">
          <button v-for="song in queue" :key="song.queueId" class="queue-item" @click="playSelected(song,true)">
            <img :src="song.artwork" :alt="song.title" /><span class="queue-copy"><strong>{{ song.title }}</strong><small>{{ song.artist }}</small></span>
